$(function() {
    ...
    $('.category_sort').on('change', function() {
        ...
        ajax = $.post(get_url('Backend/newscategory/save_cat'),post_data);
    ...
    $(document).on("click", ".cat_del", function() {
        ...
        swal({
            ...
            function(){
                var ajax = $.post(get_url('Backend/newscategory/delete/' + id));
                ajax.done(function(){
    ...
    $(document).on("submit", "#frm_category", function(e) {
        ...
        var ajax = $.post(get_url('Backend/newscategory/update'), data);
    ...
    $(document).on("click","#cat_save",function(){
        ...
        swal({
            ...
            function(){
                var ajax = $.post(get_url('Backend/newscategory/save_cat'),post_data);
                ajax.done(function(){
...
function open_category(id) {
    NProgress.start();
    var ajax = $.post(get_url('Backend/newscategory/info/' + id));
    ajax.done(function(data) {
        $("#pcid").val(data.ncid);
        $("#pcname").val(data.ncname);
        $("#pcalias").val(data.ncalias);
        $("#pcdesc").val(data.ncdesc);
        change_icon(data.ncicon);
        change_banner(data.ncbanner);
        NProgress.done();
        $("#category").modal();
    })
    ajax.fail(function() {
...
function reload_nestable() {
    $.post(get_url('Backend/newscategory/draw_nestable')).done(function(data){
