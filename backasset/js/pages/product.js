...
$(document).ready(function () {
    ...
    $(document).on('click', '.add_bbanner_success', function () {
        window.location = BASE_URL + "Backend/product"
    });
...
function add_main_img(v) {
    if($("[name='pimg[]'][value='"+v+"']").length > 0) {
        swal("Lỗi Trùng Ảnh !","Hình ảnh của bạn đã nằm trong danh sách chọn !","error");
        return;
    }
    thumb = v.replace("ino_upload/source","ino_upload/thumbs");
     $('.main_img')
     .trigger('add.owl.carousel', ['<div  class="preview_img" ><span class="img_del" carousel=".main_img">Xóa</span><input name="pimg[]" value="' + v + '" type="hidden" /><a class="item" href="'+ v +'"><img class="img_preview"  src="' + thumb + '"/></a></div>'])
     .trigger('refresh.owl.carousel');
     load_img($('.main_img'));
}
function add_detail_img(v) {
    if($("[name='pdimg[]'][value='"+v+"']").length > 0) {
        swal("Lỗi Trùng Ảnh !","Hình ảnh của bạn đã nằm trong danh sách chọn !","error");
        return;
    }
    thumb = v.replace("ino_upload/source","ino_upload/thumbs");
     $('.detail_img')
     .trigger('add.owl.carousel', ['<div  class="preview_img" ><span class="img_del" carousel=".detail_img">Xóa</span><input name="pdimg[]" value="' + v + '" type="hidden" /><a class="item" href="'+ v +'"><img class="img_preview"  src="' + thumb + '"/></a></div>'])
     .trigger('refresh.owl.carousel');
     load_img($('.detail_img'));
}
function add_self_img(v) {
    if($("[name='psimg[]'][value='"+v+"']").length > 0) {
        swal("Lỗi Trùng Ảnh !","Hình ảnh của bạn đã nằm trong danh sách chọn !","error");
        return;
    }
    thumb = v.replace("ino_upload/source","ino_upload/thumbs");
     $('.self_img')
     .trigger('add.owl.carousel', ['<div class="preview_img" ><span class="img_del" carousel=".self_img">Xóa</span><input name="psimg[]" value="' + v + '" type="hidden" /><a class="item" href="'+ v +'"><img class="img_preview"  src="' + thumb + '"/></a></div>'])
     .trigger('refresh.owl.carousel');
     load_img($('.self_img'));
}
/* END IMG SELECT */
...
$("#pcat").select2({
     ajax: {
        ...
        url: get_url('Backend/category/category'),
        dataType: 'json',
        delay: 250,
...
setTimeout(function(){
        try {
            ...
            console.log("[INOCSM] Không tìm có ảnh chính");
        }
},0);

setTimeout(function(){
    try {
        detail_img.forEach(function(v,k){
            add_detail_img(v);
        });
    }
    catch(e) {
        console.log("[INOCSM] Không tìm có ảnh chi tiết");
    }
},0);

setTimeout(function(){
    try {
        self_img.forEach(function(v,k){
            add_self_img(v);
        });
    }
    catch(e) {
...
function add_banner_img(v) {
     var template = '<tr><td><img src="'+ v + '"> </td><td></td>'
            +'<\/td><td><a class=\"btn btn-danger btn-xs waves-effect delete_rel_news\"><i class=\"material-icons\">delete_forever<\/i> Xóa<\/a><\/td><\/tr>';
        $('#news_zone').prepend(template);
        $('.table_no_news').hide();
}
...
setTimeout(function(){
    try {
         $("#ptype").select2();
            if(pcat) {
                $('#pcat').select2("trigger","select",{data:pcat});
            }
    }
    catch(e) {
        console.log(e.message);
